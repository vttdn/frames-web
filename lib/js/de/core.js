document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/de/software.json','/lib/schema/de/video.json','/lib/schema/de/faq.json','/lib/schema/de/organization.json','/lib/schema/de/webpage-homepage.json','/lib/schema/de/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
