const TWITTER_HANDLE="withframes";const VIA_TEXT="经由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh/software.json','/lib/schema/zh/video.json','/lib/schema/zh/faq.json','/lib/schema/zh/organization.json','/lib/schema/zh/webpage-homepage.json','/lib/schema/zh/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
