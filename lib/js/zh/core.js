const TWITTER_HANDLE="withframes";const VIA_TEXT="经由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh/organization.json','/lib/schema/zh/website.json','/lib/schema/zh/software.json','/lib/schema/zh/video.json','/lib/schema/zh/faq.json','/lib/schema/zh/webpage-homepage.json','/lib/schema/zh/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
const iframeHTML=`
        ...
        <div class="video-wrapper" style="position:relative;padding-top:56.25%;">
          ${iframeHTML}
        </div>`;openOverlay({type:'youtube',titleText:'播放演示视频',contentHTML});modalContainer.classList.add('large');function onYouTubeIframeAPIReady(){ytPlayer=new YT.Player('youtube-player',{events:{'onReady':(event)=>{event.target.playVideo();}}});}
