const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/tr/organization.json','/lib/schema/tr/software.json','/lib/schema/tr/video.json','/lib/schema/tr/faq.json','/lib/schema/tr/webpage-homepage.json','/lib/schema/tr/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
