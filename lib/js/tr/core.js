document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/tr/frames.json','/lib/schema/tr/faq.json','/lib/schema/tr/organization.json','/lib/schema/tr/softwarecompany.json','/lib/schema/tr/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
