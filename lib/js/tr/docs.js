const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/tr/organization.json','/lib/schema/tr/breadcrumb-docs.json','/lib/schema/tr/webpage-docs.json','/lib/schema/tr/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Mobil navigasyon">
          <ul class="flex flex-col">
            ...
            <li><a href="/tr/docs/">Dokümantasyon</a></li>
