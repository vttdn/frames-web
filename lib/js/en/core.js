document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/en/software.json','/lib/schema/en/video.json','/lib/schema/en/faq.json','/lib/schema/en/organization.json','/lib/schema/en/webpage-homepage.json','/lib/schema/en/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
