const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/en/organization.json','/lib/schema/en/website.json','/lib/schema/en/software.json','/lib/schema/en/video.json','/lib/schema/en/faq.json','/lib/schema/en/webpage-homepage.json','/lib/schema/en/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
const iframeHTML=`
        ...
        <div class="video-wrapper" style="position:relative;padding-top:56.25%;">
          ${iframeHTML}
        </div>`;openOverlay({type:'youtube',titleText:'Play video demo',contentHTML});modalContainer.classList.add('large');function onYouTubeIframeAPIReady(){ytPlayer=new YT.Player('youtube-player',{events:{'onReady':(event)=>{event.target.playVideo();}}});}
