document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/ja/frames.json','/lib/schema/ja/faq.json','/lib/schema/ja/organization.json','/lib/schema/ja/softwarecompany.json','/lib/schema/ja/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
