const TWITTER_HANDLE="withframes_jp";const VIA_TEXT="経由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/ja/organization.json','/lib/schema/ja/website.json','/lib/schema/ja/software.json','/lib/schema/ja/video.json','/lib/schema/ja/faq.json','/lib/schema/ja/webpage-homepage.json','/lib/schema/ja/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
const iframeHTML=`
        ...
        <div class="video-wrapper" style="position:relative;padding-top:56.25%;">
          ${iframeHTML}
        </div>`;openOverlay({type:'youtube',titleText:'デモ動画を再生',contentHTML});modalContainer.classList.add('large');function onYouTubeIframeAPIReady(){ytPlayer=new YT.Player('youtube-player',{events:{'onReady':(event)=>{event.target.playVideo();}}});}
