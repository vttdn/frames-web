const TWITTER_HANDLE="withframes_jp";const VIA_TEXT="経由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/ja/breadcrumb-privacy.json','/lib/schema/ja/organization.json','/lib/schema/ja/webpage-privacy.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
