const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/fi/organization.json','/lib/schema/fi/software.json','/lib/schema/fi/video.json','/lib/schema/fi/faq.json','/lib/schema/fi/webpage-homepage.json','/lib/schema/fi/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
