document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/nb/frames.json','/lib/schema/nb/faq.json','/lib/schema/nb/organization.json','/lib/schema/nb/softwarecompany.json','/lib/schema/nb/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
