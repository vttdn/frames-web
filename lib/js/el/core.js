document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/el/frames.json','/lib/schema/el/faq.json','/lib/schema/el/organization.json','/lib/schema/el/webpage-homepage.json','/lib/schema/el/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
