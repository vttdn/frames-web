const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/el/organization.json','/lib/schema/el/software.json','/lib/schema/el/video.json','/lib/schema/el/faq.json','/lib/schema/el/webpage-homepage.json','/lib/schema/el/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
