const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/el/organization.json','/lib/schema/el/breadcrumb-docs.json','/lib/schema/el/webpage-docs.json','/lib/schema/el/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Πλοήγηση κινητού">
          <ul class="flex flex-col">
            ...
            <li><a href="/el/docs/">Τεκμηρίωση</a></li>
