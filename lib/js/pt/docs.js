const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/pt/organization.json','/lib/schema/pt/breadcrumb-docs.json','/lib/schema/pt/webpage-docs.json','/lib/schema/pt/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Navegação móvel">
          <ul class="flex flex-col">
            ...
            <li><a href="/pt/docs/">Documentação</a></li>
