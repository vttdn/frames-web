const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/pt/organization.json','/lib/schema/pt/software.json','/lib/schema/pt/video.json','/lib/schema/pt/faq.json','/lib/schema/pt/webpage-homepage.json','/lib/schema/pt/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
