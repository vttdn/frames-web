document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/es/frames.json','/lib/schema/es/faq.json','/lib/schema/es/organization.json','/lib/schema/es/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
overlay.addEventListener('click',(e)=>{if(e.target===overlay)closeOverlay();});document.addEventListener('keydown',(e)=>{if(e.key==='Escape'&&!overlay.hidden)closeOverlay();});const socialBtn=document.getElementById('button-social-share');if(socialBtn){socialBtn.dataset.overlayTrigger='social';socialBtn.addEventListener('click',()=>{const contentHTML=`
        ...
        </div>`;openOverlay({type:'social',titleText:'Compartir',contentHTML});modalContainer.querySelector('.overlay-button-close').addEventListener('click',closeOverlay);});}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Navegación móvil">
          <ul class="flex flex-col">
            <li><a href="https://withframes.com/es/#features">Funciones</a></li>
            <li><a href="/docs/">Documentación</a></li>
            <li><a href="/es/changelog/">Changelog</a></li>
            <li><a href="/es/confidencialidad/">Privacidad</a></li>
          </ul>
        </nav>
      </div>
    `;openOverlay({type:'menu',titleText:'Navegación móvil',contentHTML});modalContainer.classList.add('modal-menu');const closeBtn=modalContainer.querySelector('.overlay-button-close');if(closeBtn){closeBtn.addEventListener('click',()=>{modalContainer.classList.remove('modal-menu');closeOverlay();});}
...
const playBtn=document.getElementById('youtube-video-toggle');if(playBtn){playBtn.dataset.overlayTrigger='youtube';playBtn.addEventListener('click',()=>{const videoId=playBtn.dataset.videoId;if(!videoId)return;const iframeHTML=`
      ...
      <div class="video-wrapper" style="position:relative;padding-top:56.25%;">
        ${iframeHTML}
      </div>`;openOverlay({type:'youtube',titleText:'Reproducir vídeo demo',contentHTML});modalContainer.classList.add('large');modalContainer.querySelector('.overlay-button-close').addEventListener('click',()=>{modalContainer.classList.remove('large');closeOverlay();});});}});function shareOn(platform){const url=encodeURIComponent(window.location.href);const title=encodeURIComponent(document.title);const text=encodeURIComponent("Frames es una app fotografía analógica. Registra cada foto, guarda ajustes, organiza carretes y conserva metadatos EXIF en iPhone & Mac.");let shareUrl='';switch(platform){case'facebook':shareUrl=`https://www.facebook.com/sharer/sharer.php?u=${url}`;break;case'twitter':shareUrl=`https://twitter.com/intent/tweet?url=${url}&text=${text}`;break;case'linkedin':shareUrl=`https://www.linkedin.com/sharing/share-offsite/?url=${url}`;break;case'whatsapp':shareUrl=`https://wa.me/?text=${text}%20${url}`;break;case'reddit':shareUrl=`https://reddit.com/submit?url=${url}&title=${title}`;break;case'email':shareUrl=`mailto:?subject=${title}&body=${text}%20${url}`;break;}
