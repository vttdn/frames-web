const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/es/organization.json','/lib/schema/es/breadcrumb-docs.json','/lib/schema/es/webpage-docs.json','/lib/schema/es/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Navegación móvil">
          <ul class="flex flex-col">
            ...
            <li><a href="/es/docs/">Documentación</a></li>
