document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/pl/software.json','/lib/schema/pl/video.json','/lib/schema/pl/faq.json','/lib/schema/pl/organization.json','/lib/schema/pl/webpage-homepage.json','/lib/schema/pl/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
