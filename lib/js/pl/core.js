document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/pl/frames.json','/lib/schema/pl/faq.json','/lib/schema/pl/organization.json','/lib/schema/pl/softwarecompany.json','/lib/schema/pl/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
