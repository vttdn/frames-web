const TWITTER_HANDLE="withframes";const VIA_TEXT="przez";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/pl/organization.json','/lib/schema/pl/breadcrumb-docs.json','/lib/schema/pl/webpage-docs.json','/lib/schema/pl/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Nawigacja mobilna">
          <ul class="flex flex-col">
            ...
            <li><a href="/pl/docs/">Dokumentacja</a></li>
