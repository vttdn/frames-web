const TWITTER_HANDLE="withframes";const VIA_TEXT="przez";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/pl/breadcrumb-privacy.json','/lib/schema/pl/organization.json','/lib/schema/pl/webpage-privacy.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
