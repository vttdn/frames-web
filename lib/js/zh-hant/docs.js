const TWITTER_HANDLE="withframes";const VIA_TEXT="經由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh-hant/organization.json','/lib/schema/zh-hant/breadcrumb-docs.json','/lib/schema/zh-hant/webpage-docs.json','/lib/schema/zh-hant/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="行動導航">
          <ul class="flex flex-col">
            ...
            <li><a href="/zh-hant/docs/">文件</a></li>
