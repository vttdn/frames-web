const TWITTER_HANDLE="withframes";const VIA_TEXT="經由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh-hant/organization.json','/lib/schema/zh-hant/website.json','/lib/schema/zh-hant/software.json','/lib/schema/zh-hant/video.json','/lib/schema/zh-hant/faq.json','/lib/schema/zh-hant/webpage-homepage.json','/lib/schema/zh-hant/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
const iframeHTML=`
        ...
        <div class="video-wrapper" style="position:relative;padding-top:56.25%;">
          ${iframeHTML}
        </div>`;openOverlay({type:'youtube',titleText:'播放演示影片',contentHTML});modalContainer.classList.add('large');function onYouTubeIframeAPIReady(){ytPlayer=new YT.Player('youtube-player',{events:{'onReady':(event)=>{event.target.playVideo();}}});}
