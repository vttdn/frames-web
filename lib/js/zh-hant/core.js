document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh-hant/frames.json','/lib/schema/zh-hant/faq.json','/lib/schema/zh-hant/organization.json','/lib/schema/zh-hant/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
overlay.addEventListener('click',(e)=>{if(e.target===overlay)closeOverlay();});document.addEventListener('keydown',(e)=>{if(e.key==='Escape'&&!overlay.hidden)closeOverlay();});const socialBtn=document.getElementById('button-social-share');if(socialBtn){socialBtn.dataset.overlayTrigger='social';socialBtn.addEventListener('click',()=>{const contentHTML=`
          ...
          <div class="modal-social-buttons grid grid-3col">
            ...
            <a href="#" onclick="shareOn('email');return false;">
              <img src="/lib/img/icn/paperplane.svg" alt="電子郵件 Icon" width="14" height="14" loading="lazy">
            </a>
        </div>`;openOverlay({type:'social',titleText:'分享',contentHTML});modalContainer.querySelector('.overlay-button-close').addEventListener('click',closeOverlay);});}
const menuBtn=document.getElementById('menu-toggle');if(menuBtn){menuBtn.dataset.overlayTrigger='menu';menuBtn.addEventListener('click',()=>{const contentHTML=`
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="行動導航">
          <ul class="flex flex-col">
            <li><a href="https://withframes.com/zh-hant/#features">功能特色</a></li>
            <li><a href="/docs/">文件</a></li>
            <li><a href="/zh-hant/changelog/">更新日誌</a></li>
            <li><a href="/zh-hant/privacy/">隱私</a></li>
          </ul>
        </nav>
      </div>
    `;openOverlay({type:'menu',titleText:'行動導航',contentHTML});modalContainer.classList.add('modal-menu');const closeBtn=modalContainer.querySelector('.overlay-button-close');if(closeBtn){closeBtn.addEventListener('click',()=>{modalContainer.classList.remove('modal-menu');closeOverlay();});}
