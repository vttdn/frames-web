const TWITTER_HANDLE="withframes";const VIA_TEXT="經由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh-hant/software.json','/lib/schema/zh-hant/video.json','/lib/schema/zh-hant/faq.json','/lib/schema/zh-hant/organization.json','/lib/schema/zh-hant/webpage-homepage.json','/lib/schema/zh-hant/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
