document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh-hant/frames.json','/lib/schema/zh-hant/faq.json','/lib/schema/zh-hant/organization.json','/lib/schema/zh-hant/softwarecompany.json','/lib/schema/zh-hant/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
