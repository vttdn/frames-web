const TWITTER_HANDLE="withframes";const VIA_TEXT="經由";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/zh-hant/breadcrumb-privacy.json','/lib/schema/zh-hant/organization.json','/lib/schema/zh-hant/webpage-privacy.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
