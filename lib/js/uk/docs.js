const TWITTER_HANDLE="withframes";const VIA_TEXT="через";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/uk/organization.json','/lib/schema/uk/breadcrumb-docs.json','/lib/schema/uk/webpage-docs.json','/lib/schema/uk/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Мобільна навігація">
          <ul class="flex flex-col">
            ...
            <li><a href="/uk/docs/">Документація</a></li>
