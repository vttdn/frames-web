const TWITTER_HANDLE="withframes";const VIA_TEXT="через";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/uk/breadcrumb-privacy.json','/lib/schema/uk/organization.json','/lib/schema/uk/webpage-privacy.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
