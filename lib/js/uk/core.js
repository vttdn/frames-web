const TWITTER_HANDLE="withframes";const VIA_TEXT="через";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/uk/organization.json','/lib/schema/uk/software.json','/lib/schema/uk/video.json','/lib/schema/uk/faq.json','/lib/schema/uk/webpage-homepage.json','/lib/schema/uk/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
