document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/uk/frames.json','/lib/schema/uk/faq.json','/lib/schema/uk/organization.json','/lib/schema/uk/softwarecompany.json','/lib/schema/uk/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
