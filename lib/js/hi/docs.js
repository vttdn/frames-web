const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/hi/organization.json','/lib/schema/hi/breadcrumb-docs.json','/lib/schema/hi/webpage-docs.json','/lib/schema/hi/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="मोबाइल नेविगेशन">
          <ul class="flex flex-col">
            ...
            <li><a href="/hi/docs/">दस्तावेज़ीकरण</a></li>
