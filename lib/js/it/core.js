const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/it/organization.json','/lib/schema/it/website.json','/lib/schema/it/software.json','/lib/schema/it/video.json','/lib/schema/it/faq.json','/lib/schema/it/webpage-homepage.json','/lib/schema/it/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
const iframeHTML=`
        ...
        <div class="video-wrapper" style="position:relative;padding-top:56.25%;">
          ${iframeHTML}
        </div>`;openOverlay({type:'youtube',titleText:'Riproduci video demo',contentHTML});modalContainer.classList.add('large');function onYouTubeIframeAPIReady(){ytPlayer=new YT.Player('youtube-player',{events:{'onReady':(event)=>{event.target.playVideo();}}});}
