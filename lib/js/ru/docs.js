const TWITTER_HANDLE="withframes";const VIA_TEXT="через";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/ru/organization.json','/lib/schema/ru/breadcrumb-docs.json','/lib/schema/ru/webpage-docs.json','/lib/schema/ru/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Мобильная навигация">
          <ul class="flex flex-col">
            ...
            <li><a href="/ru/docs/">Документация</a></li>
