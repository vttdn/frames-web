document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/sv/frames.json','/lib/schema/sv/faq.json','/lib/schema/sv/organization.json','/lib/schema/sv/webpage-homepage.json','/lib/schema/sv/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
