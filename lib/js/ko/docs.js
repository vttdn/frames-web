const TWITTER_HANDLE="withframes_kr";const VIA_TEXT="통해";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/ko/organization.json','/lib/schema/ko/breadcrumb-docs.json','/lib/schema/ko/webpage-docs.json','/lib/schema/ko/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
