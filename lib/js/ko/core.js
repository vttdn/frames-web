const TWITTER_HANDLE="withframes_kr";const VIA_TEXT="통해";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/ko/software.json','/lib/schema/ko/video.json','/lib/schema/ko/faq.json','/lib/schema/ko/organization.json','/lib/schema/ko/webpage-homepage.json','/lib/schema/ko/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
