document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/nl/frames.json','/lib/schema/nl/faq.json','/lib/schema/nl/organization.json','/lib/schema/nl/softwarecompany.json','/lib/schema/nl/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
