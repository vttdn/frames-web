const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/nl/software.json','/lib/schema/nl/video.json','/lib/schema/nl/faq.json','/lib/schema/nl/organization.json','/lib/schema/nl/webpage-homepage.json','/lib/schema/nl/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
