const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/nl/organization.json','/lib/schema/nl/breadcrumb-docs.json','/lib/schema/nl/webpage-docs.json','/lib/schema/nl/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Mobiele navigatie">
          <ul class="flex flex-col">
            ...
            <li><a href="/nl/docs/">Documentatie</a></li>
