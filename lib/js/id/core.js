const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/id/organization.json','/lib/schema/id/software.json','/lib/schema/id/video.json','/lib/schema/id/faq.json','/lib/schema/id/webpage-homepage.json','/lib/schema/id/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
