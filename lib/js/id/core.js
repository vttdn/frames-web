document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/id/frames.json','/lib/schema/id/faq.json','/lib/schema/id/organization.json','/lib/schema/id/softwarecompany.json','/lib/schema/id/webpage-homepage.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
