const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/da/organization.json','/lib/schema/da/breadcrumb-docs.json','/lib/schema/da/webpage-docs.json','/lib/schema/da/video.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
      ...
      <div class="wrapper">
        ...
        <nav class="mobile-menu-nav" aria-label="Mobilnavigation">
          <ul class="flex flex-col">
            ...
            <li><a href="/da/docs/">Dokumentation</a></li>
