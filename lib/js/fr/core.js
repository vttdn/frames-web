document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/fr/software.json','/lib/schema/fr/video.json','/lib/schema/fr/faq.json','/lib/schema/fr/organization.json','/lib/schema/fr/webpage-homepage.json','/lib/schema/fr/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
