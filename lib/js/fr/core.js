const TWITTER_HANDLE="withframes";const VIA_TEXT="via";document.addEventListener('DOMContentLoaded',function(){const schemaFiles=['/lib/schema/fr/organization.json','/lib/schema/fr/website.json','/lib/schema/fr/software.json','/lib/schema/fr/video.json','/lib/schema/fr/faq.json','/lib/schema/fr/webpage-homepage.json','/lib/schema/fr/howto.json'];schemaFiles.forEach((url)=>{fetch(url).then(response=>{if(!response.ok){throw new Error(`Failed to fetch ${url}: ${response.status}`);}
...
const iframeHTML=`
        ...
        <div class="video-wrapper" style="position:relative;padding-top:56.25%;">
          ${iframeHTML}
        </div>`;openOverlay({type:'youtube',titleText:'Lire la vidéo de démonstration',contentHTML});modalContainer.classList.add('large');function onYouTubeIframeAPIReady(){ytPlayer=new YT.Player('youtube-player',{events:{'onReady':(event)=>{event.target.playVideo();}}});}
